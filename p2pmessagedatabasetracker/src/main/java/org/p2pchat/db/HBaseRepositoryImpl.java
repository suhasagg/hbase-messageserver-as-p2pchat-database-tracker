...
import org.p2pchat.hbase.HBaseService;
import org.p2pchat.model.Message;
import org.p2pchat.model.Room;
import org.p2pchat.model.RoomMeta;
    ...
    public void addMessages(int roomId, List<Message> messages){
        try {
            ...
            for (Message message : messages) {
                ...
                put.addColumn(Bytes.toBytes(MESSAGE_MESSAGE_COLUMN_FAMILY), Bytes.toBytes("peerId"), Bytes.toBytes(message.getPeerId()));
    ...
    public List<Message> getMessages(int roomId, long startTime, long endTime) {
        try {
            ...
            for (Result result : results) {
                String author = Bytes.toString(result.getValue(Bytes.toBytes(MESSAGE_MESSAGE_COLUMN_FAMILY), Bytes.toBytes("peerId")));
